import QtQuick

Item {
    id: root

    readonly property color transparent: Qt.rgba(0, 0, 0, 0)
    readonly property color white: Qt.rgba(255 / 255, 255 / 255, 255 / 255, 1)
    readonly property color mainBackground: Qt.rgba(38 / 255, 38 / 255, 38 / 255, 1)
    readonly property color tabEnable: Qt.rgba(59 / 255, 59 / 255, 59 / 255, 1)
    readonly property color tabHoverEnable: Qt.rgba(48 / 255, 48 / 255, 48 / 255, 1)
    readonly property color disable: Qt.rgba(166 / 255, 166 / 255, 166 / 255, 1)
    readonly property color line: Qt.rgba(46 / 255, 46 / 255, 46 / 255, 1)
}
